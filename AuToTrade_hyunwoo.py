...
while True :
    try :
        ...
        for coin in coin_name : # 현재 사기 가장 좋은 코인 고르기
            ...
            if ror <= 1 or get_check_coin(coin) == 0 : # 수익률이 101% 이하 이거나 전날대비 5% 이하인 코인은 패스
                continue
        ...
        if max_coin != None : # 조건에 맞는 코인 있을 때
            ...
            print(max_coin,get_current_price(max_coin),get_target_price(max_coin, k),get_ma6(max_coin),max_ror,ch,buy_cnt)
            ...
            if start_time < now < end_time - datetime.timedelta(minutes=3) :
                ...
                if target_price <= current_price and ma6 < current_price and ch == 0 and buy_cnt > 0 :
